...
AnyPick manages multiple native logins for local AI tools.
...
- inspect and repair AnyPick-owned operational state.

```text
official tool login
       ↓
live native auth ← anypick → saved account snapshots
       ↓                         ↓
...
AnyPick's pointer to the saved account expected to correspond to live auth. Live identity
...
AnyPick does not automate the official login flow.
...
- diagnose missing auth, stale processes, permissions, and incomplete AnyPick operations.
...
- Does live auth match AnyPick's active record?
- Which proxy is running and on what endpoint?
- What exactly happens if I anypick to another account?
