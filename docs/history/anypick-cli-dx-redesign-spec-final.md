# AnyPick CLI DX Redesign Specification

**Status:** Canonical implementation specification  
**Revision:** 2.4 — recovery serialization and source-specific planner steps  
**Supersedes:** `anypick-cli-dx-redesign-spec.md`
...
The first redesign correctly identified that AnyPick should expose user intent rather than internal subsystems, but it still introduced too many user-facing abstractions and inconsistent command grammar.
...
anypick run claude
anypick run claude --with grok/work
anypick current
```
...
```text
use  = set the persistent AnyPick-managed default
...
Normal users do not manage contexts. AnyPick stores internal bindings. A reusable preset exists only when the user explicitly asks to save one.

---

# 2. What the product promises

AnyPick does not promise that users need to understand no concepts at all. A CLI invocation must identify what is being operated on.

AnyPick promises that users do not need to classify an operation by implementation subsystem:
...
`anypick run <client>` runs only an effective AnyPick binding:

1. explicit temporary `--with`;
2. project binding;
3. global AnyPick binding.

It must not silently use an unmanaged native configuration.

Unmanaged configuration may be inspected by `anypick current` and diagnosed by `anypick doctor`, but is never an implicit `run` fallback.
...
```bash
anypick use claude --with grok/work --save work-grok
```
...
AnyPick automates proxy lifecycle only when:
...
anypick preset ...
anypick proxy ...
anypick auth ...       legacy/advanced
anypick profile ...    deprecated gateway alias
```
...
```text
anypick add context
anypick edit context
anypick list contexts
anypick context use
```
...
Set or change a persistent AnyPick-managed binding for a client.
...
anypick use codex --with openrouter-work
anypick use claude --with @work-grok
anypick use claude --with grok/work --model grok-4.5
anypick use claude --with grok/work --save work-grok
anypick use claude --current
anypick use claude --with grok/work --dry-run
```
...
1. load the stored global AnyPick binding for the client;
...
This TTY shorthand is equivalent to `anypick use claude --current` only when a global binding exists.
...
Launch a client using its effective AnyPick binding, with an optional process-scoped override.

## 9.2 Syntax

```bash
anypick run <CLIENT> [--with <SOURCE|@PRESET>] [--model MODEL] [--] [CLIENT_ARGS...]
```
...
anypick run claude --with @work-grok
anypick run codex --with openrouter-work -- --model gpt-5
```
...
For `anypick run <CLIENT>` without `--with`:

1. project binding for that client;
2. global AnyPick binding for that client;
3. error.
...
```text
No AnyPick source is configured for Claude.
...
Existing native Claude configuration was detected, but AnyPick will not use it implicitly.
```
...
Show the effective AnyPick-managed state and separately report unmanaged configuration.

## 10.2 Syntax

```bash
anypick current [CLIENT] [--json]
```

`anypick status` remains a compatibility alias.

## 10.3 Human output

```text
Claude
  AnyPick source  grok/work
  Model          grok-4.5
...
```text
Claude
  AnyPick source  openrouter-work
...
The labels must prevent users from confusing external config with a AnyPick binding.
...
```bash
anypick list [accounts|gateways|clients|presets] [FILTER] [--json]
```
...
`anypick add account codex` is ambiguous when:
...
```bash
anypick preset list
anypick preset show work-grok
anypick preset edit work-grok [options]
anypick preset rename work-grok work
anypick preset remove work-grok
```

Generic management also works:

```bash
anypick edit @work-grok
anypick remove @work-grok
```
...
When a preset is used, AnyPick resolves it into a concrete binding snapshot. Editing the preset later affects future `use`, `run --with`, or `link --with` operations only. It must not silently change existing global or project bindings.
...
```bash
anypick link <CLIENT>
anypick link <CLIENT> --with <SOURCE|@PRESET> [--model MODEL]
...
Store project bindings in AnyPick’s database. Do not write secrets or AnyPick config into the repository.

## 16.3 `anypick link <client>` without `--with`

`anypick link <client>` without `--with` resolves a snapshot in this order:

1. current global AnyPick binding for that client;
...
```bash
anypick link claude --with @work-grok
```

AnyPick must resolve the preset once and store a concrete project-binding snapshot. It must not store a live preset reference required for execution.
...
Remove AnyPick-managed configuration for a client while preserving external/native configuration unless AnyPick has an exact tracked backup to restore.

## 17.2 Syntax

```bash
anypick reset [CLIENT] [--dry-run]
```
...
- remove only AnyPick-managed config blocks;
...
```bash
anypick doctor [TARGET] [--fix] [--json]
```
...
- occupied ports;
- stale locks/PIDs;
- mismatched AnyPick binding and client configuration;
...
1. delete a stale AnyPick-owned lock after verifying the owner process is absent;
2. delete a stale AnyPick-owned PID record after verifying the process is absent or unrelated;
3. stop a AnyPick-owned orphan proxy only when:
   - no active global/project binding references it;
   - no live lease references it;
   - ownership marker matches;
4. delete a AnyPick-owned temporary overlay only when:
   - no live process references it;
   - ownership marker and age checks pass;
5. repair permissions only on AnyPick-owned files and directories;
...
Review:
  anypick current claude --verbose

Reset only AnyPick-managed configuration:
...
AnyPick contains and owns the proxy implementation.

Example: built-in Grok/OpenCode proxy support where already implemented.

### managed_external_proxy

AnyPick can locate, launch, monitor, and stop an installed external executable through an adapter.

Example: Kiro only if a supported `kirolink`/`kiro-proxy` executable is installed and the adapter contract is implemented.

### external_manual_proxy

The source requires an external proxy but AnyPick cannot safely own its lifecycle.

AnyPick must stop before mutating the client and provide setup guidance.
...
AnyPick did not find a supported proxy executable.
...
1. an explicit AnyPick adapter configuration path, when configured;
...
- `explicit`: AnyPick resolved and stored a concrete model ID;
- `omitted`: AnyPick intentionally leaves model selection to a verified client/source default;
...
```text
AnyPick — use your AI accounts and gateways with any supported CLI

Usage:
  anypick <command> [options]

Common:
  use       Set what a client uses by default
  run       Launch a client
  current   Show effective AnyPick bindings
  list      List accounts and gateways
  add       Add an account or gateway
  reset     Remove AnyPick-managed configuration
...
Troubleshooting:
  doctor    Diagnose setup; auto-fix only safe AnyPick-owned state
...
```json
{
  "error": {
    ...
    "message": "No AnyPick source is configured for Claude.",
    "client": "claude",
    "suggestions": [
      "anypick use claude --with grok/work",
      "anypick run claude --with grok/work"
...
|---|---|
| `anypick status` | `anypick current` |
| `anypick whoami` | `anypick current` / `anypick account current` |
| `anypick switch codex personal` | `anypick use codex --with codex/personal` |
| `anypick save codex work` | `anypick add account codex --current --name work` |
| `anypick stash codex` | hidden inside `anypick add account codex --new` |
| `anypick refresh codex` | `anypick account refresh codex` |
| `anypick apply openrouter-work -c codex` | `anypick use codex --with openrouter-work` |
| `anypick profile create ...` | `anypick add gateway ...` |
| `anypick profile edit ...` | `anypick edit gateway/...` |
| `anypick profile reset --client claude` | `anypick reset claude` |
...
Examples of acceptable evidence include a legacy per-client active-account record, an exact AnyPick-owned native-auth marker, or an adapter fingerprint that uniquely matches one saved account.
...
- allow the user to capture it later through `anypick add account --current` and `anypick use`.
...
3. an exact AnyPick-owned configuration marker;
4. an endpoint mapped to one unique AnyPick-owned proxy/source;
5. an adapter fingerprint that exactly matches one saved account or gateway.

If the source cannot be identified exactly:

- create no AnyPick binding;
...
1. old AnyPick per-client apply metadata;
2. an explicit model in the effective client configuration;
3. a AnyPick-tracked environment/config overlay.
...
`anypick current` must render it as:

```text
Model  unknown · migrated legacy state
```

`anypick use <client>` in TTY, `anypick use <client> --current`, and `anypick run <client>` must not fail solely because the migrated model is unknown when the client/source adapter verifies that omission is safe. In that case, preserve the unknown state and execute without injecting a model.
...
Choose one explicitly:
  anypick use claude --with openrouter-work --model anthropic/claude-sonnet-4
...
4. `run claude` errors with no AnyPick binding despite detected native config;
...
- `current` clearly separates AnyPick-managed and external state.
...
```text
Implement the canonical redesign in anypick-cli-dx-redesign-spec-final.md.
...
6. `run <client>` resolves only project then global AnyPick bindings.
...
14. `anypick add account` hides stash/save mechanics and supports explicit --current/--new modes.
...
Everything involving auth snapshots, API profiles, protocol conversion, proxy lifecycle, ports, environment variables, and configuration files is AnyPick’s responsibility.
