/**
 * The hotplug preview must not promise something `accounts.use()` does not do.
 *
 * `buildHotplugPreview` (src/tui/model/pool.ts) is what the TUI shows before an
 ...
 *
 * Note this previews `AccountService.use()` (hotplug the machine's live login for
...
import { buildHotplugPreview } from '../src/tui/model';
import { FakeProvider } from './helpers';

describe('hotplug preview fidelity', () => {
  let root: string;
  let app: HotplugApp;
  let proxied: FakeProvider;

  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'hotplug-preview-'));
  ...
  it('reports alreadyActive for the account that is already live', async () => {
    ...
    const preview = await buildHotplugPreview(app, 'plain', 'alpha');
  ...
  it('does not promise a proxy start when the proxy is disabled', async () => {
    ...
    const preview = await buildHotplugPreview(app, 'withproxy', 'beta');
  ...
  it('says no pre-switch steps are needed when nothing has to be undone', async () => {
    ...
    const preview = await buildHotplugPreview(app, 'plain', 'beta');
  ...
  it('still reports "already active" alongside a proxy note', async () => {
    ...
    const preview = await buildHotplugPreview(app, 'withproxy', 'alpha');
    expect(preview.alreadyActive).toBe(true);
    expect(preview.steps.notes.some((n) => /already active/i.test(n))).toBe(true);
  });

  it('throws for an account that does not exist', async () => {
    await expect(buildHotplugPreview(app, 'plain', 'nope')).rejects.toThrow(/not found/i);
  });
});
